...
import { openAiEmbeddingsRequiresApiKey, resolveOpenAiEmbeddingsUrl, } from './openai-embeddings-url';
...
const CONFIG_PATH = process.env.AGENT_ENGRAMS_CONFIG ||
...
export function loadConfig() {
    ...
    switch (providerType) {
        case 'openai': {
            const baseUrl = envStr('AGENT_ENGRAMS_OPENAI_BASE_URL') ??
                (file?.provider?.type === 'openai' ? file.provider.baseUrl : undefined);
            const apiKey = envStr('AGENT_ENGRAMS_OPENAI_API_KEY') ??
            ...
            provider = {
                type: 'openai',
                apiKey: apiKey?.trim() ?? '',
                model: envStr('AGENT_ENGRAMS_OPENAI_MODEL') ??
        ...
        case 'bedrock':
            provider = {
                type: 'bedrock',
                profile: envStr('AGENT_ENGRAMS_BEDROCK_PROFILE') ??
                    (file?.provider?.type === 'bedrock' ? file.provider.profile : undefined) ??
                    'default',
                region: envStr('AGENT_ENGRAMS_BEDROCK_REGION') ??
                    (file?.provider?.type === 'bedrock' ? file.provider.region : undefined) ??
                    'us-east-1',
                model: envStr('AGENT_ENGRAMS_BEDROCK_MODEL') ??
        ...
        case 'ollama':
            provider = {
                type: 'ollama',
                url: envStr('AGENT_ENGRAMS_OLLAMA_URL') ??
                    (file?.provider?.type === 'ollama' ? file.provider.url : undefined) ??
                    'http://localhost:11434',
                model: envStr('AGENT_ENGRAMS_OLLAMA_MODEL') ??
